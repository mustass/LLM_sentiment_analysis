...
class LitSaModel(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for metric in self.metrics:
            score = self.metrics[metric](logits, label)
            self.log(f'val_{metric}', score, on_step=True, on_epoch=True, prog_bar=True, logger=True)
    ...
    def test_step(self, batch, batch_idx):
        ...
        for metric in self.metrics:
            score = self.metrics[metric](logits, label)
            self.log(f'test_{metric}', score, on_step=True, on_epoch=True, prog_bar=True, logger=True)